<div>
    @php
        ...
        $estimatedDate = $mailPhase?->installation_date ?? $order->delivery_date;
    @endphp
    <p>Good day,</p>
    <p>We expect that the delivery and installation of your order <strong>{{$order->name}}{{$phaseLabel}}</strong> are estimated to occur during the week of <strong>{{ \Carbon\Carbon::parse($estimatedDate)->startOfWeek()->format('m-d-Y') }}</strong> to <strong>{{ \Carbon\Carbon::parse($estimatedDate)->endOfWeek()->format('m-d-Y') }}</strong>.</p>
    ...
    <hr/>
   <p>Buen día,</p>
    <p>Esperamos que la entrega e instalación de su orden <strong>{{$order->name}}{{$phaseLabel}}</strong> se estimen para la semana del <strong>{{ \Carbon\Carbon::parse($estimatedDate)->startOfWeek()->format('d-m-Y') }}</strong> al <strong>{{ \Carbon\Carbon::parse($estimatedDate)->endOfWeek()->format('d-m-Y') }}</strong>.</p>
