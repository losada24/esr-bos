<div>
    <p>Good day,</p>
    <p>We expect that your material for order <strong>{{$order->name}}</strong> arrives on <strong>{{ \Carbon\Carbon::parse($order->delivery_date)->format('m-d-Y')}}</strong>.</p>
    ...
    <hr/>
    <p>Buen día,</p> 
    <p>Esperamos que su material para el pedido <strong>{{$order->name}}</strong> llegue el <strong>{{ \Carbon\Carbon::parse($order->delivery_date)->format('m-d-Y')}}</strong>.</p> 
