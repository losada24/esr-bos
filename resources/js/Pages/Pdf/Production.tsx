      ...
      >
        ...
        <PrintLayout>
          <Page wrap size="LETTER" style={tw('p-6 font-regular')}>
            ...
            }} documentTitle='Production Cost #' />
            <ReportCompany order={order} />
            {order?.products?.map((product, index) => {
              return <ReportProduct product={product} key={index} showPrices={true} roles={auth.user.roles.map((role) => role.name)} isImpactGlass={IS_IMPACT_GLASS} />
            })}
