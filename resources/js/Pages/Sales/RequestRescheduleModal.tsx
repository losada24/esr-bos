    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-slate-800">Move to REQUEST RE-SCHEDULE</h3>
            <p className="mt-2 text-sm text-slate-600">
              Provide a note to move order <strong>{taskTitle}</strong> to <strong>REQUEST RE-SCHEDULE</strong>.
            </p>
          </div>
          <button
            type="button"
        ...
        <Formik<RequestRescheduleFormValues>
          enableReinitialize
          initialValues={{
            note: initialNote ?? '',
          }}
          validate={(values) => {
            const issues: Partial<Record<keyof RequestRescheduleFormValues, string>> = {}
