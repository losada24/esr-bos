...
import { getGlassWidth, getGlassHeight } from '@/Utils/SingleHung'
...
export const singleHuntSchema = Yup.object({
  ...
  height: Yup.number().required('Height is required').min(20.5, 'Min height 20.5').max(77, 'Max height 77'),
  width: Yup.number().required('Width is required').min(18.5, 'Min width 18.5').max(54, 'Max width 54'),
  ...
  low_e: Yup.string().when(['order_glass_type'], {
    is: (order_glass_type: string) => order_glass_type !== RUSH_GLASS_TYPE,
    then: Yup.string().required('Glass coating required').max(255, 'Max 255 characters'),
    otherwise: Yup.string().nullable().max(255, 'Max 255 characters')
  }),
