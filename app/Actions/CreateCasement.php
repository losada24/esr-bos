...
class CreateCasement {

  public function handle(Request $request) {
    DB::transaction(function() use ($request) {
      $casementProduct = new CasementProduct(
        $request->width,
        $request->height,
        $request->frame_color,
        $request->config,
      ...
      $unitPrice = $casementProduct->getUnitPrice();
      ...
      $product = Product::create([
        ...
        'system' => ExternalProductEnum::$CASEMENT,
        'width' => $request->width,
      ...
      if( !$product )
      {
          throw new \Exception('Casement not created');
      }
    });
  }
}
