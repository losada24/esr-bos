...
trait OrderEmails {
  ...
  public function sendEmail(Order $order) {
    if ($order->status === OrderStatusEnum::PLANNED->value) {
      ...
      array_merge($users, $accountManager->pluck('email')->toArray());
      ...
      if ($order->service === Service::INSTALLATION->value) {
        ...
        array_merge($users, $usersByRoleManager->pluck('email')->toArray());
        foreach ($users as $user) {
        ...
        array_merge($users, $serviceManager->pluck('email')->toArray());
        foreach ($users as $user) {
          Mail::to($user)->send(new InstallationDateConfirmation($order, true, true));
        }
        $user = $order->installationTeams->pluck('user.email')->toArray();
        $accountManager = User::role([RoleEnum::ACCOUNT_MANAGER->value])->get();
        array_merge($users, $accountManager->pluck('email')->toArray());
        foreach ($users as $user) {
