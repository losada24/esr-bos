...
trait Reports
{
  ...
  public function getOrdersByInstaller($id, $status = null, $startDate = null, $endDate = null, $orderStatus = null)
  {
    ...
    return $orders->map(function ($order, $key) {
      ...
      $installerPayment = $order->installationPayments->sum('installer_payment');
      $pendingPaymentAmount = $amount - (float) $installerPayment;
      $totalPaymentAmount = 0;
      $lastInstallerPaymentAmount = $order->installationPayments->last()->installer_payment ?? 0;
      $lastExtraWork = $order->installationPayments->last()->extra_work ?? 0;
      $lastExtraDiscount = $order->installationPayments->last()->extra_discount ?? 0;
      $lastOtherCostInstaller = $order->installationPayments->last()->other_cost_installer ?? 0;
      $totalPaymentAmount = $lastInstallerPaymentAmount + $lastExtraWork - $lastExtraDiscount + $lastOtherCostInstaller;
