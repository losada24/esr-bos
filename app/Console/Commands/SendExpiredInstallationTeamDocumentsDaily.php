...
class SendExpiredInstallationTeamDocumentsDaily extends Command
{
    ...
    public function handle()
    {
        $installationTeams = InstallationTeam::with('user')
          ->where('worker_compensation_expiration_date', '>=', Carbon::now()->subDays(30))
          ->orWhere('liability_expiration_date', '>=', Carbon::now()->subDays(30))
          ->get();
        ...
        foreach ($installationTeams as $installationTeam) {
          ...
          Mail::to($email_user)->send(new InstallationTeamExpireDocuments($installationTeam));
        }
    }
}
